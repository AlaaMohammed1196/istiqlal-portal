...
class ProfileController extends Controller
{
    ...
    {
        $user = getUserData();
        if($user['code']==401) return redirect()->route('portal.v2.logout');
        $data = $user['data']['USER'][0];

        return view('portal_v2.profile.index', compact('data'));
    }
    ...
    public function changePassword(Request $request){
        ...
        $apiURL = config('app.api').'/PortalUsersApi/api/auth/ChangePassword?MODULE_ID='.Session::get('userData')['MODULE_ID'];
    ...
    public function edit(Request $request)
    {
        ...
        return view('portal_v2.profile.create', compact('countries', 'genders', 'nationalities', 'jobs', 'data'));
    }
    ...
    public function update(Request $request){
        ...
        $apiURL = config('app.api').'/PortalUsersApi/api/auth/UpdateUser?MODULE_ID='.Session::get('userData')['MODULE_ID'];
        ...
        $status = $responseBody['status'];
        $msg = $responseBody['message'];
        $url = route('portal.v2.profile.index');

        if($request->USER_PICTURE){
            $path = generalUpload('attachments', $request->USER_PICTURE);
            if($path){
                deleteUpload($path);
                $apiURL = config('app.api_attach').'/PortalUsersApi/api/auth/UpdateUserPicture?MODULE_ID='.Session::get('userData')['MODULE_ID'];
    ...
    public function getConstants(){
        $apiURL = config('app.api').'/PortalUsersApi/api/auth/GetUserConstants?MODULE_ID='.Session::get('userData')['MODULE_ID'];
    ...
    public function updatePhone(Request $request){
        ...
        $apiURL = config('app.api').'/PortalUsersApi/api/auth/UpdateUserCelular?MODULE_ID='.Session::get('userData')['MODULE_ID'];

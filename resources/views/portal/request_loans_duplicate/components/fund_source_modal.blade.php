                ...
                <div class="row">
                    <div class="col-12 col-sm-9 col-md-9 col-lg-9">
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control float-only" name="ANNUAL_SOURCE_VALUE" placeholder=" " />
                            <label>القيمة</label>
                        </div>
                    </div>
            ...
            <form id="edit_fund_source" action="{{route('portal.loan-request.fund-source.store')}}">
                <div class="modal-body wizard" id="wizardBasic">
                    ...
                    <div class="row">
                        <div class="col-12 col-sm-9 col-md-9 col-lg-9">
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control float-only" name="ANNUAL_SOURCE_VALUE" id="source_ANNUAL_SOURCE_VALUE" placeholder=" " />
                                <label>القيمة</label>
                            </div>
                        </div>
                        <div class="col-12 col-sm-3 col-md-3 col-lg-3">
                            <input type="text" name="SOURCE_CURR_ID" value="{{$constants['DefualtCurr']}}" hidden>
                            <div class="form-floating mb-3 w-100">
                                <select class="select-floating-no-search" id="SOURCE_CURR_ID" disabled>
                                    <option></option>
                                    @foreach($constants['CURRENCIES'] as $index=>$item)
                                        <option value="{{$item['VALUE']}}" {{$constants['DefualtCurr']==$item['VALUE']?'selected':''}}>{{$item['LABEL']}}</option>
                                    @endforeach

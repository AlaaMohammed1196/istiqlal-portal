                    ...
                    <div class="row">
                        <div class="col-12 col-sm-9 col-md-9 col-lg-9">
                            <div class="form-floating mb-4">
                                <input type="text" class="form-control float-only" placeholder="القيمة" name="ANNUAL_SOURCE_VALUE"/>
                                <label>القيمة</label>
                            </div>
                        </div>
                        <div class="col-12 col-sm-3 col-md-3 col-lg-3">
                            <input type="text" name="SOURCE_CUST_CONTR_CURR_ID" value="{{$constants['DefualtCurr']}}" hidden>
                            <div class="form-floating mb-4 w-100">
                                <select class="select-single-with-search" disabled>
                                    <option></option>
            ...
            <form id="edit_fund_desc" action="{{route('portal.loan-request.fund-desc.store')}}">
                <div class="modal-body wizard" id="wizardBasic">
                    ...
                    <div class="row">
                        <div class="col-12 col-sm-9 col-md-9 col-lg-9">
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control float-only" placeholder="القيمة" name="ANNUAL_SOURCE_VALUE" id="ANNUAL_SOURCE_VALUE"/>
                                <label>القيمة</label>
                            </div>
                        </div>
                        <div class="col-12 col-sm-3 col-md-3 col-lg-3">
                            <div class="form-floating mb-3 w-100">
                                <input type="text" name="SOURCE_CUST_CONTR_CURR_ID" value="{{$constants['DefualtCurr']}}" hidden>
                                <select class="select-floating-no-search" id="SOURCE_CUST_CONTR_CURR_ID" disabled>
                                    <option></option>
                                    @foreach($constants['CURRENCIES'] as $index=>$item)
                                        <option value="{{$item['VALUE']}}" {{$constants['DefualtCurr']==$item['VALUE']?'selected':''}}>{{$item['LABEL']}}</option>
                                    @endforeach

                ...
                <tr class="transfer_order" id="order2-{{$item['VOUCHER_SEQ']}}">
                    ...
                    <td>
                        <div class="d-flex justify-content-center">
                            <a href="javascript:void(0);" role="button" data-type="2" data-seq="{{$item['VOUCHER_SEQ']}}" data-status="{{$item['APPROVAL_STATUS_ID']}}" class="request_details btn btn-sm btn-icon btn-icon-only btn-outline-secondary align-top ms-1"
                               data-bs-toggle="tooltip" data-bs-title="عرض التفاصيل">
                                <i class="fa-solid fa-eye"></i>
                            </a>
                            @if($item['IS_UPDATABLE'] == 1)
                                <a href="{{route('portal.v2.orders.transfer.edit', $item['VOUCHER_SEQ'])}}" class="btn btn-sm btn-icon btn-icon-only btn-outline-dark align-top ms-1"
                                   data-bs-toggle="tooltip" data-bs-title="تعديل">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>
                            @endif
                            @if(!in_array(1, array_column(Session::get('userData')['USER_ROLES'], 'ROLE_ID')))
                            <a href="javascript:void(0);" role="button" data-status="{{$item['APPROVAL_STATUS_ID']}}" data-type="2" data-seq="{{$item['VOUCHER_SEQ']}}" data-id="" class="step_request btn btn-sm btn-icon btn-icon-only btn-outline-secondary align-top ms-1"
                               data-bs-toggle="tooltip" data-bs-title="خطوات الطلب">
                                <i class="fa-solid fa-list"></i>
                            </a>
                            @endif
                            <button type="button" data-seq="{{$item['VOUCHER_SEQ']}}" class="transfer_to_pdf btn btn-sm btn-icon btn-icon-only btn-outline-secondary align-top ms-1"
                                    data-bs-toggle="tooltip" data-bs-title="طباعة الطلب">
                                <i class="fa-solid fa-print"></i>
                            </button>
                            @if($item['APPROVAL_STATUS_ID'] == 1 && $item['IS_CANCELABLE'] == 1)
                                <a href="javascript:void(0);" role="button" data-answer="4" data-type="2" data-seq="{{$item['VOUCHER_SEQ']}}" data-id="" class="reject_request btn btn-sm btn-icon btn-icon-only btn-outline-danger align-top ms-1"

<div class="modal-body py-2" id="modal_info">
    <input type="text" name="TICKET_SEQ" value="{{$extra['TICKET_SEQ']}}" hidden>
                                ...
                                <tr class="{{$role['APPROVAL_STATUS_ID']!=1?'':'d-none'}}">
                                    ...
                                    <td>
                                        @if($role['USER_HAVE_ROLE'] && $role['APPROVAL_STATUS_ID']!=1)
                                            <a href="javascript:void(0);" role="button" data-type="3" data-seq="{{$extra['TICKET_SEQ']}}" data-step="{{$step['STEP_ID']}}" data-role="{{$role['ROLE_ID']}}" class="undo_step_request btn btn-sm btn-icon btn-icon-only btn-outline-danger align-top mx-1"

                ...
                <tr class="transfer_order" id="order3-{{$item['TICKET_SEQ']}}">
                    ...
                    <td>
                        <div class="d-flex justify-content-center">
                            <a href="javascript:void(0);" role="button" data-type="3" data-seq="{{$item['TICKET_SEQ']}}" data-status="{{$item['APPROVAL_STATUS_ID']}}" class="request_details btn btn-sm btn-icon btn-icon-only btn-outline-secondary align-top ms-1"
                               data-bs-toggle="tooltip" data-bs-title="عرض التفاصيل">
                                <i class="fa-solid fa-eye"></i>
                            </a>
                            @if(!in_array(1, array_column(Session::get('userData')['USER_ROLES'], 'ROLE_ID')))
                            <a href="javascript:void(0);" role="button" data-status="{{$item['APPROVAL_STATUS_ID']}}" data-type="3" data-seq="{{$item['TICKET_SEQ']}}" data-id="{{$item['TICKET_ID']}}" class="step_request btn btn-sm btn-icon btn-icon-only btn-outline-secondary align-top ms-1"
                            ...
                            @if($item['APPROVAL_STATUS_ID'] == 1 && in_array(1, array_column(Session::get('userData')['USER_ROLES'], 'ROLE_ID')))
                                <a href="javascript:void(0);" role="button" data-answer="4" data-type="3" data-seq="{{$item['TICKET_SEQ']}}" data-id="{{$item['TICKET_ID']}}" class="reject_request btn btn-sm btn-icon btn-icon-only btn-outline-danger align-top ms-1"

                                ...
                                <tr class="{{$role['APPROVAL_STATUS_ID']!=1?'':'d-none'}}">
                                    ...
                                    <td>
                                        @if($role['USER_HAVE_ROLE'] && $role['APPROVAL_STATUS_ID']!=1)
                                            <a href="javascript:void(0);" role="button" data-type="1" data-seq="{{$extra['BENEFICIARY_SEQ']}}" data-step="{{$step['STEP_ID']}}" data-role="{{$role['ROLE_ID']}}" class="undo_step_request btn btn-sm btn-icon btn-icon-only btn-outline-danger align-top mx-1"

                    ...
                    <div class="card-header border-0 pb-0">
                        <ul class="nav nav-tabs nav-tabs-line card-header-tabs responsive-tabs" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link {{!isset($tab)?'active':''}}" data-bs-toggle="tab" data-bs-target="#first" role="tab" type="button" aria-selected="true">المستفيدين</button>
                            </li>
                            <li class="nav-item" role="presentation">
...
{{--                            <li class="nav-item" role="presentation">--}}
{{--                                <button class="nav-link {{$tab=='checks'?'active':''}}" data-bs-toggle="tab" data-bs-target="#fifth" role="tab" type="button" aria-selected="false">دفتر الشيكات</button>--}}
{{--                            </li>--}}
                    ...
                    <div class="card-body position-relative">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade {{!isset($tab)?'active show':''}}" id="first" role="tabpanel">
                                @include('portal_v2.orders.beneficiaries.index')
                            </div>
                            <div class="tab-pane fade {{$tab=='transfers'?'active show':''}}" id="second" role="tabpanel">
                                @include('portal_v2.orders.transfers.index')
                            </div>
                            <div class="tab-pane fade {{$tab=='tickets'?'active show':''}}" id="third" role="tabpanel">
                                @include('portal_v2.orders.tickets.index')
                            </div>
                            <div class="tab-pane fade {{$tab=='deposits'?'active show':''}}" id="fourth" role="tabpanel">
                                @include('portal_v2.orders.deposits.index')
                            </div>
{{--                            <div class="tab-pane fade {{$tab=='checks'?'active show':''}}" id="fifth" role="tabpanel">--}}
{{--                                @include('portal_v2.orders.checks.index')--}}
{{--                            </div>--}}
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header py-4">
                        <h5 class="modal-title fw-bold" id="addnewLabel">الإجازات</h5>
    ...
    <script>
        ...
        $(document).on('click', '.step_request', function (e){
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response.status){
                        $('#confirm_modal #confirm_form').html(response.html);
        ...
        $(document).on('click', '#agree, #disagree', function (e){
            e.preventDefault();
            let form = $('#confirm_form');
            let formData = new FormData(document.getElementById('confirm_form'))
            formData.append('APPROVAL_STATUS', $(this).attr('data-APPROVAL_STATUS'))
            loaderStart(form)
            $.ajax({
                headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                type: "POST",
        ...
        $(document).on('click', '#return_request', function (e){
            e.preventDefault();
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                        url: '{{route('portal.v2.orders.return')}}',
                        type: 'POST',
                        data: {
                            'seq': seq,
                            'type': type,
